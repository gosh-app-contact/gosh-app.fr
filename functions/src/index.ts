...
export const closeExpiredGoshOffs = onSchedule(
  {
  ...
  async () => {
    ...
    if (snap.empty) { console.log('[closeExpiredGoshOffs] Aucun GoshOff expiré.'); return; }

    console.log(`[closeExpiredGoshOffs] ${snap.size} GoshOff(s) à clôturer.`);
    const batch = db.batch();
    for (const docSnap of snap.docs) {
      const g = docSnap.data();
      const ct: number = g.challengerTonnage ?? 0;
      const dt: number = g.challengedTonnage ?? 0;
      let winnerId: string | null = null;
      if (ct > dt) winnerId = g.challengerClubId;
      else if (dt > ct) winnerId = g.challengedClubId;
...
export const recalculateTonnage = functions
  .region('europe-west1')
  ...
  .onWrite(async (change) => {
    ...
    if (prsAfter === prsBefore) return;

    const prs = Object.entries(after.prs ?? {}) as Array<[string, { uid: string; exerciseSlug: string; weight: number }]>;
    ...
    if (fraudulent.length > 0) {
      ...
      for (const [key] of fraudulent) patch[`prs.${key}`] = admin.firestore.FieldValue.delete();
    ...
    const challengerMembers: string[] = cSnap.data()?.memberIds ?? [];
    const challengedMembers: string[] = dSnap.data()?.memberIds ?? [];

    const validPrs = prs.map(([, p]) => p);
    const calcTonnage = (memberIds: string[]) =>
      validPrs.filter((p) => memberIds.includes(p.uid)).reduce((sum, p) => sum + p.weight, 0);
...
async function getFcmTokens(clubId: string): Promise<string[]> {
  const clubSnap = await db.collection('clubs').doc(clubId).get();
  const memberIds: string[] = clubSnap.data()?.memberIds ?? [];
  if (memberIds.length === 0) return [];
  const userSnaps = await Promise.all(memberIds.map((uid) => db.collection('users').doc(uid).get()));
  return userSnaps.map((s) => s.data()?.fcmToken).filter(Boolean) as string[];
}
...
export const goshOffEvents = functions
  .region('europe-west1')
  .firestore.document('goshoffs/{goshOffId}')
  .onWrite(async (change) => {
    const after = change.after.exists ? change.after.data()! : null;
    ...
    if (statusBefore === 'active' && statusAfter === 'finished') {
      ...
      let cm: string; let dm: string;
      if (!after.winnerId) { cm = `Match nul contre ${after.challengedClubName} !`; dm = `Match nul contre ${after.challengerClubName} !`; }
      else if (after.winnerId === after.challengerClubId) { cm = `Victoire contre ${after.challengedClubName} ! 🏆`; dm = `Défaite contre ${after.challengerClubName}. Bonne chance la prochaine fois.`; }
      else { cm = `Défaite contre ${after.challengedClubName}. Bonne chance la prochaine fois.`; dm = `Victoire contre ${after.challengerClubName} ! 🏆`; }
